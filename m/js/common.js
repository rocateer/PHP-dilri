...
function COM_set_timer(set_minute,id){
  ...
  var timer = setInterval(function () {
    ...
    var_time =  temp_minute +":"+temp_second;
...
function file_upload_click(img_id,file_type,limit_cnt,width,height,accept_file,img_resize){
  ...
  var fileUpload = "<input type='file' name='file[]' id='ex_file' accept='"+accept_file+" ' onchange=\"file_upload('"+img_id+"','"+file_type+"','"+limit_cnt+"','"+width+"','"+height+"','"+accept_file+"','"+img_resize+"');\" style='display:none' ><input type='hidden' name='img_resize' id='img_resize' value='"+img_resize+"'>";
...
function file_upload(img_id,file_type,limit_cnt,width,height,accept_file,img_resize){
  ...
  $.ajax({
    url         : "/kr/common/multi_fileUpload",
    type        : 'post',
    dataType    : 'json',
    processData : false,
    contentType : false,
    ...
    success     : function(img_list){
      ...
      for(var i = 0; i < img_list.length; i++){
       ...
       str+="</li>";
        ...
        }else if(img_id=='board_img'){
          set_one_img(img_list[i].path);
        }else if(img_id=='member_img'){
          set_one_img(img_list[i].path);
        }else if(img_id=='chatting_img'){
          set_img(img_list[i].path);
          // set_corp_img(img_list[i].path);
        }else{
          $('#'+img_id).append(str);
        }
...
//파일업로드요청:서버->앱
function api_request_file_upload(img_id, file_cnt, lang_code){
  console.log(app_yn);
  console.log(agent);
  img_id_val = img_id;
  ...
  if (file_cnt) {
    if ($("."+img_id+"_div").length>=file_cnt) {
      ...
      } else if (lang_code=='bd') {
        alert("lang_member_info_00814");

      } else {
        alert("Registration can possible maximum "+file_cnt);
...
function api_reponse_file_upload(file_path){
  if (img_id_val=='board_img') {
    set_one_img(file_path);
  }else if (img_id_val=='member_img') {
    set_one_img(file_path);
  }else {
    set_img(file_path);
  }
