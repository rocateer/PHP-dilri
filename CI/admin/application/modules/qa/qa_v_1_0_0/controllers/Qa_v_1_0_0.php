...
class Qa_v_1_0_0 extends MY_Controller{
	...
	// 관리자 확인
	public function check_reg_in(){
		$qa_idx = $this->_input_check("qa_idx",array("empty_msg"=>"QA 코드 누락"));

		$data['qa_idx'] = $qa_idx;

		$result = $this->model_qa->check_reg_in($data); // qa 댓글 삭제
	...
	public function qa_reply_reg_in(){
		$qa_idx = $this->_input_check("qa_idx",array("empty_msg"=>"QA 코드 누락"));
		$reply_contents = $this->_input_check("reply_contents",array("empty_msg"=>"답변 내용을 작성후 답변 등록 버튼을 눌러 주세요."));
		...
		if($result == "0") {
			...
			//알람::106 문의하신 1:1 문의에 답변이 작성 되었습니다.
			$check = $this->model_qa->qa_detail($data);
			$index="106";
