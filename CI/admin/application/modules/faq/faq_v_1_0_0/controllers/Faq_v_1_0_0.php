...
class Faq_v_1_0_0 extends MY_Controller{
	...
	public function faq_mod_up(){
		...
		$result = $this->model_faq->faq_mod_up($data);//faq 수정하기
	...
	// 공지사항 상태 변경
	public function faq_state_mod_up(){
	$faq_idx = $this->_input_check("faq_idx",array("empty_msg"=>"공지사항 키가 누락되었습니다."));
	$state = $this->_input_check("state",array("empty_msg"=>"공지사항 상태 코드가 누락되었습니다."));

	$data['faq_idx']  = $faq_idx;
	$data['state'] = $state;

	$result = $this->model_faq->faq_state_mod_up($data); // 공지사항 상태 변경
