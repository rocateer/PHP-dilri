...
?>
	<li class="relative chatting_li chatting_room_<?=$row->chatting_room_idx?> chatting_new" id="chatting_room_<?=$row->chatting_room_idx?>">
					...
					<div class="img_box profile">
						<? if($row->member_idx==$this->member_idx){ ?>
							...
							<? if($row->member_read_yn=='N'){ ?>
								<span class="new" id="new_img_<?=$row->chatting_room_idx?>"></span>
							<? } ?>
						<? } else {?>
	...
	<script type="text/javascript">
		arr.push('<?=$row->chatting_room_idx?>');
		console.log(arr);
	</script>
<?php
		}
