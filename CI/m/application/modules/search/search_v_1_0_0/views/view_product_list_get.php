				...
				<table class="tbl_4">
					...
					<tr>
						...
						<? } else {?>
							<a href="javascript:void(0)" onclick="COM_profile_check('<?=$this->member_idx?>','<?=$return_url_str?>','<?=$fnc_str?>')" >
						<? } ?>
								<div class="s_info"> <?=$this->global_function->convert_time_exp($row->list_up_date)?> · <?=lang("lang_main_00131","나와의 거리")?> <?=$row->distance?>km</div>
							</a>
						</td>
					</tr>
					<tr>
...
$("#product_filter").css("display","<?=$display2?>");
