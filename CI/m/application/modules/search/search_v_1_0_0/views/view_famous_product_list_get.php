				...
				<td>
					<? if($row->member_idx==$this->member_idx){ ?>
						...
						<? } else {?>
							<a href="javascript:void(0)" onclick="COM_profile_check('<?=$this->member_idx?>','<?=$return_url_str?>','<?=$fnc_str?>')" >
						<? } ?>
						<div class="s_info"> <?=$this->global_function->convert_time_exp($row->list_up_date)?> · <?=lang("lang_search_00381","나와의 거리")?> <?=$row->distance?>km</div>
					</a>
				</td>
			</tr>
			<tr>
					...
					<? if($row->member_idx==$this->member_idx){ ?>
						...
						<div class="price">
							<?if($row->product_state == '1'){?>
								<span class="state"><?=lang("lang_mypage_00535","예약")?></span>
							<?} else if($row->product_state == '2'){?>
								<span class="state comp"><?=lang("lang_mypage_00536","완료")?></span>
							<?}?>
				...
				<td style="vertical-align:bottom">
					<ul class="info_ul">
						...
						<li  >
							...
							$fnc_str = "product_like_reg_in2(\'".$row->product_idx."\',this)";
							?>
							<span id="_like_span_<?=$row->product_idx?>" onclick="COM_profile_check('<?=$this->member_idx?>','<?=$return_url_str?>','<?=$fnc_str?>')" class="like like_1 <?=$row->like_yn=='Y'?'on':''?>"></span>
							<span id="_like_cnt_<?=$row->product_idx?>">
