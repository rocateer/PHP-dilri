...
class Common extends MY_Controller {
	...
	// 프로필 입력 체크
	public function COM_profile_check(){
		...
		$result = $this->model_common->COM_profile_check($data); // 회원 정보 수정

		if(empty($result->member_phone) || empty($result->member_name) ){
			$response->code = '0';
			$response->code_msg = '프로필 정보 미입력';
		} else {
			$response->code = '1';
			$response->code_msg = '프로필 정보 입력 완료';
		}
...
//해시태그 리스트
	public function keyword_sub_list(){
		...
		$data_array=$this->model_common->keyword_sub_list($data); 	//해시태그 리스트
