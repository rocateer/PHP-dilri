  ...
  public function member_search($data){
    ...
    if($index =="101" ){
      ...
      $sql = "SELECT
              ...
              WHERE
                a.del_yn='N'
                AND '$title' REGEXP REPLACE(a.alarm_keyword,',','|')
                AND member_idx NOT IN ($except_member_idx)
      ";
      ...
      foreach ($member_list as $row) {
        $str_arr = explode(',', $row->alarm_keyword);
        for ($i=0; $i < count($str_arr); $i++) {
          if (strpos($title, $str_arr[$i]) !== false) {
            $row->keyword = $str_arr[$i];
    ...
    if($index =="102" || $index =="103" || $index =="104"|| $index =="105" || $index =="106" || $index =="109" || $index =="112"){
      $sql = "SELECT
                a.member_idx,
                a.current_lang,
              ...
              WHERE
                ...
                AND a.member_idx IN (
                                    SELECT
                                      z.member_idx
                                    FROM
                                      tbl_product_review z
                                      JOIN tbl_product b ON b.product_idx = z.product_idx
                                    WHERE
                                      z.del_yn='N'
                                      AND DATE_SUB(NOW(), INTERVAL 8 DAY) <= DATE_FORMAT(z.ins_date, '%Y-%m-%d')
                                      AND DATE_SUB(NOW(), INTERVAL 1 DAY) >= DATE_FORMAT(z.ins_date, '%Y-%m-%d')
                                      AND b.free_product_yn = '$free_product_yn'
                )
      ";
