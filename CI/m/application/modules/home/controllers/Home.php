...
class Home extends MY_Controller {
  ...
  public function index(){
    ...
    $uuid = $this->_input_check("uuid",array());

    set_cookie('app_yn', $app_yn, 3600*24*365);
    $gcm_key = get_cookie('gcm_key');
    $device_os = get_cookie('device_os');
    $current_nation = !empty(get_cookie('current_nation'))?get_cookie('current_nation'):'bd';
      ...
      if($member_idx>0){
        ...
        if(count($result)>0){
          if($result->del_yn =="N"){
            ...
            set_cookie('app_yn', $app_yn, 3600*24*365);
            set_cookie('current_nation', $current_nation, 3600*24*365);
            set_cookie('uuid', $uuid, 3600*24*365);
